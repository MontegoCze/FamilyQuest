...
export default function App() {
  ...
  if (!token) return <AuthScreen mode={authMode} setMode={setAuthMode} onLogin={(nextToken, nextUser) => { localStorage.setItem('familyquest_token', nextToken); setToken(nextToken); setUser(nextUser); }} error={error} setError={setError} />;
  if (loading) return <div className="loading"><div className="loading-spinner">✦</div><span>Načítám FamilyQuest…</span></div>;
  if (!user) return <AuthScreen mode={authMode} setMode={setAuthMode} onLogin={(nextToken, nextUser) => { localStorage.setItem('familyquest_token', nextToken); setToken(nextToken); setUser(nextUser); }} error={error} setError={setError} />;
...
function ParentDashboard({ user, token, onLogout }: { user: User; token: string; onLogout: () => void }) {
  ...
  const [adventurePoints, setAdventurePoints] = useState<AdventurePoint[]>([]);
  ...
  const refresh = async () => {
    try {
      const [nextTasks, nextStats, nextMembers, nextNotifications, nextAchievements, nextFamily, nextRewards, nextRedemptions, nextLeaderboard, nextAdventurePoints] = await Promise.all([
        ...
        request<Reward[]>('/rewards', {}, token), request<Redemption[]>('/redemptions', {}, token), request<Stats[]>('/leaderboard', {}, token), request<AdventurePoint[]>('/adventure', {}, token),
      ]);
      setTasks(nextTasks); setStats(nextStats); setMembers(nextMembers); setNotifications(nextNotifications); setAchievements(nextAchievements); setFamily(nextFamily); setRewards(nextRewards); setRedemptions(nextRedemptions); setLeaderboard(nextLeaderboard); setAdventurePoints(nextAdventurePoints);
  ...
  return <main className="app-shell" id="top"><DashboardSidebar user={user} familyName={family?.name ?? 'Vaše rodina'} onLogout={onLogout} /><div className="dashboard-main"><header className="mobile-header"><a className="sidebar-brand" href="#top"><span className="logo small">✦</span><strong>FamilyQuest</strong></a><button className="header-avatar" aria-label="Profil">{user.avatar || user.full_name.charAt(0)}</button></header><div className="dashboard-content"><header className="page-heading"><div><p className="eyebrow">Rodičovský přehled</p><h1>Ahoj, {user.full_name.split(' ')[0]} <span className="heading-wave">👋</span></h1><p className="muted">Tady je dnešní přehled rodiny.</p></div><div className="heading-actions"><button className="secondary" onClick={() => setShowMemberForm(!showMemberForm)}>+ Přidat člena rodiny</button><button className="primary" onClick={() => setShowTaskForm(!showTaskForm)}>+ Nový úkol</button></div></header>{message && <p className="notice">{message}</p>}  {(showTaskForm || editingTask) && <TaskForm token={token} children={children} task={editingTask} onDone={() => { setShowTaskForm(false); setEditingTask(null); refresh(); }} />}{showMemberForm && <MemberForm token={token} onDone={() => { setShowMemberForm(false); refresh(); }} />}{loading ? <DashboardSkeleton /> : <><section className="stats-grid parent-stats" id="stats"><Stat label="Členové rodiny" value={String(members.length)} icon="♧" tone="purple" /><Stat label="Splněné úkoly" value={String(completedToday)} icon="✓" tone="green" /><Stat label="Získané XP" value={String(totalXp)} icon="✦" tone="amber" /><Stat label="Čeká na hodnocení" value={String(pendingReviews.length)} icon="◷" tone="rose" /></section><div className="parent-main-grid"><section className="panel pending-panel" id="missions"><div className="panel-heading"><div><p className="eyebrow">Vyžaduje pozornost</p><h2>Čeká na tvoje hodnocení <span className="count-badge">{pendingReviews.length}</span></h2></div><span className="panel-icon rose">◷</span></div>{pendingReviews.length === 0 ? <div className="empty-state compact-empty"><span>✓</span><strong>Všechno je ohodnocené!</strong><p>Nic na tebe právě nečeká.</p></div> : <div className="review-list">{pendingReviews.map(({ task, completion, name }) => <article className="review-card" key={completion.id}><span className="user-avatar child-avatar">{name.charAt(0)}</span><div className="review-info"><strong>{name}</strong>  <h3>{task.title}</h3><small><CategoryBadge category={task.category} /> Dokončeno nedávno · +{task.xp} XP</small></div><div className="review-buttons"><button className="approve" onClick={() => review(completion.id, 'approved')}>✓ Schválit</button><button className="reject" onClick={() => review(completion.id, 'rejected')}>Vrátit</button></div></article>)}</div>}</section><aside className="parent-side"><section className="panel family-card"><div className="panel-heading"><div><p className="eyebrow">Vaše posádka</p><h2>Děti</h2></div><span className="panel-icon purple">♧</span></div>{children.length === 0 ? <div className="empty">Zatím nemáte přidané děti.</div> : <div className="family-list">{children.map((child) => { const childStats = leaderboard.find((item) => item.user_id === child.user_id); const childTasks = tasks.filter((task) => task.assignments.some((assignment) => assignment.user_id === child.user_id)); const done = childTasks.filter((task) => task.completions.some((completion) => completion.user_id === child.user_id && completion.status === 'approved')).length; return <article className="family-member" key={child.user_id}><span className="user-avatar child-avatar">{(child.full_name ?? '?').charAt(0)}</span><div className="member-info"><strong>{child.full_name ?? child.email}</strong><small>Úroveň {childStats?.level ?? 1} · 🔥 {childStats?.current_streak ?? 0} dní</small><div className="member-progress"><span style={{ width: `${childStats?.xp_in_level ?? 0}%` }} /></div><small>{done}/{childTasks.length || 0} dnešních úkolů · {childStats?.total_xp ?? 0} XP</small></div></article>; })}</div>}</section><section className="panel activity-card"><div className="panel-heading"><div><p className="eyebrow">Rodinný rytmus</p><h2>Aktivita rodiny</h2></div><span className="panel-icon green">◒</span></div><div className="activity-chart" aria-label="Přehled dokončených úkolů"><div className="chart-bars">{['Po', 'Út', 'St', 'Čt', 'Pá', 'So', 'Ne'].map((day, index) => <div className="chart-column" key={day}><span style={{ height: `${Math.max(16, Math.min(100, (completedToday + index * 7) % 92))}%` }} /><small>{day}</small></div>)}</div><p className="chart-caption"><strong>{completedToday}</strong> splněných úkolů v přehledu</p></div></section></aside>  </div><section className="panel family-members-panel"><div className="panel-heading"><div><p className="eyebrow">Rodina</p><h2>👨‍👩‍👧‍👦 Členové rodiny</h2></div><button className="secondary" onClick={() => setShowMemberForm(true)}>+ Přidat člena</button></div><div className="family-members-grid">{members.map((member) => <article className="family-member-card" key={member.user_id}><span className="user-avatar child-avatar">{member.avatar || (member.full_name ?? '?').charAt(0)}</span><div><strong>{member.full_name ?? member.email}</strong><small>{member.role === 'parent' ? '👑 Rodič' : '🧒 Dítě'} · {member.email}</small></div><div className="member-actions"><button className="secondary compact" onClick={() => setShowMemberForm(true)}>Upravit</button>{member.user_id !== user.id && <button className="danger compact" onClick={() => removeMember(member)}>Odebrat</button>}</div></article>)}</div></section>  <AccountManager token={token} members={members} currentUserId={user.id} onRefresh={refresh} /><section className="parent-lower-grid" id="rewards"><section className="panel pending-rewards"><div className="panel-heading"><div><p className="eyebrow">Odměny</p><h2>Čekající žádosti</h2></div><span className="panel-icon amber">♢</span></div>{redemptions.filter((item) => item.status === 'pending').length === 0 ? <div className="empty-state compact-empty"><span>✓</span><strong>Žádné čekající odměny</strong><p>Vše je vyřešené.</p></div> : redemptions.filter((item) => item.status === 'pending').map((redemption) => <article className="redemption-row" key={redemption.id}><span className="reward-icon">🎁</span><div><strong>{members.find((member) => member.user_id === redemption.user_id)?.full_name ?? 'Člen rodiny'}</strong><small>{redemption.reward_name ?? rewards.find((reward) => reward.id === redemption.reward_id)?.name} · {redemption.reward_cost ?? rewards.find((reward) => reward.id === redemption.reward_id)?.cost} XP</small></div><button className="approve compact" onClick={() => reviewReward(redemption.id, 'approved')}>Schválit</button><button className="reject compact" onClick={() => reviewReward(redemption.id, 'rejected')}>Zamítnout</button></article>)}</section><section className="panel notifications-panel"><div className="panel-heading"><div><p className="eyebrow">Přehled</p><h2>Oznámení</h2></div><span className="notification-count-inline">{notifications.filter((item) => !item.is_read).length}</span></div>{notifications.slice(0, 4).map((item) => <button className={`notification ${item.is_read ? '' : 'unread'}`} key={item.id} onClick={() => markRead(item.id)}><span>{item.is_read ? '○' : '●'}</span><span><strong>{item.title}</strong><small>{item.message}</small></span></button>)}{notifications.length === 0 && <div className="empty">Žádná nová oznámení.</div>}</section></section>  <section className="panel all-tasks-panel"><div className="panel-heading"><div><p className="eyebrow">Prehled</p><h2>Vsechny rodinne ukoly</h2></div><div className="category-filters">{[['all','Vše'],['home','🏠 Doma'],['school','🎒 Škola']].map(([value,label]) => <button key={value} className={categoryFilter === value ? 'active' : ''} onClick={() => setCategoryFilter(value as 'all' | TaskCategory)}>{label}</button>)}</div></div>{visibleTasks.length === 0 ? <div className="empty">Zatim nemate zadne ukoly.</div> : <div className="task-overview-list">{visibleTasks.map((task) => <article className="task-overview-row" key={task.id}><span className="task-overview-icon">{categoryInfo[normalizeCategory(task.category)].icon}</span><div><strong>{task.title}</strong><small><CategoryBadge category={task.category} /> {task.assignments.map((assignment) => assignment.user_name ?? assignment.user_id).join(", ")} · +{task.xp} XP</small></div>  <span className="task-overview-status"><button className="secondary compact" onClick={() => setEditingTask(task)}>Upravit</button> {task.completions.some((completion) => completion.status === "approved") ? "? Hotovo" : task.completions.some((completion) => completion.status === "pending") ? "Ceka na hodnoceni" : "Ceka"}</span></article>)}</div>}</section><AchievementManager achievements={achievements} token={token} onChange={refresh} /><AdventureManager points={adventurePoints} token={token} onChange={refresh} /></>}</div></div><MobileNav role="parent" /></main>;
}
...
function AdventureMap({ points, totalXp, level }: { points: AdventurePoint[]; totalXp: number; level: number }) {
  ...
  return <section className="adventure-map-panel" id="adventure"><div className="child-section-heading"><div><p className="eyebrow">🗺️ MOJE DOBRODRUŽSTVÍ</p><h2>Cesta hrdiny</h2></div><span className="adventure-xp">{totalXp} XP · LVL {level}</span></div>{points.length === 0 ? <div className="child-empty"><span>🧭</span><strong>Mapa se teprve připravuje</strong><p>Rodiče brzy přidají první dobrodružný bod.</p></div> : <><div className="adventure-map-canvas" aria-label="Mapa dobrodružství">{points.map((point) => <button key={point.id} className={`adventure-point ${point.status}`} style={{ left: `${point.position_x}%`, top: `${point.position_y}%` }} onClick={() => setSelected(point)} aria-label={`${point.title}, ${point.status}`}><span>{point.status === 'locked' ? '🔒' : point.status === 'completed' ? '✓' : point.icon}</span><small>{point.title}</small></button>)}</div>{selected && <article className="adventure-detail"><button className="modal-close" onClick={() => setSelected(null)} aria-label="Zavřít">×</button><span className="adventure-detail-icon">{selected.status === 'locked' ? '🔒' : selected.icon}</span><h3>{selected.title}</h3><p>{selected.description || 'Další zastávka na tvé cestě.'}</p><small>{selected.status === 'locked' ? `🔒 Ještě potřebuješ ${Math.max(0, selected.required_xp - totalXp)} XP.` : selected.status === 'completed' ? '✅ Tento bod je dokončený.' : '🟢 Toto je tvůj aktuální bod.'}</small>{selected.required_level && <small>Úroveň {selected.required_level} · </small>}{selected.reward_xp > 0 && <small>🎁 Odměna +{selected.reward_xp} XP</small>}</article>}</>}</section>;
}
...
function MemberForm({ token, onDone }: { token: string; onDone: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (role === 'parent') {
        const invite = await request<{ token: string }>('/family/invitations', { method: 'POST', body: JSON.stringify({ full_name: data.get('full_name'), email: data.get('email') }) }, token);
        setInviteToken(invite.token);
      } else {
        await request('/family/members', { method: 'POST', body: JSON.stringify({ role, full_name: data.get('full_name'), email: data.get('email') || null, password: data.get('password') }) }, token);
        onDone();
      }

    } catch (err) { setError(err instanceof Error ? err.message : 'Člena se nepodařilo přidat.'); }
  };
  return <form className="task-form member-form" onSubmit={submit}><fieldset className="member-role-picker"><legend>Koho chcete přidat?</legend><button type="button" className={role === 'parent' ? 'selected' : ''} onClick={() => setRole('parent')}>👑<strong>Rodič</strong><small>Správce rodiny</small></button><button type="button" className={role === 'child' ? 'selected' : ''} onClick={() => setRole('child')}>🧒<strong>Dítě</strong><small>Člen dětské části</small></button></fieldset><input name="full_name" required minLength={2} placeholder="Jméno / přezdívka" /><input name="email" type="email" required={role === 'parent'} placeholder={role === 'parent' ? 'E-mail rodiče' : 'E-mail (volitelné)'} />{role === 'child' && <input name="password" type="password" required minLength={8} placeholder="Heslo pro přihlášení (8+ znaků)" />}{error && <p className="error">{error}</p>}{inviteToken ? <p className="notice">Pozvánka vytvořena. Předejte rodiči tento kód: <strong>{inviteToken}</strong></p> : <button className="primary" type="submit">{role === 'parent' ? 'Vytvořit pozvánku' : 'Přidat dítě'}</button>}{inviteToken && <button className="secondary" type="button" onClick={onDone}>Hotovo</button>}</form>;
}
