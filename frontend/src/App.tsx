...
function MemberForm({ token, onDone }: { token: string; onDone: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === 'invite') {
        const invite = await request<Invitation>('/family/invitations', { method: 'POST', body: JSON.stringify({ email: data.get('email'), role }) }, token);
