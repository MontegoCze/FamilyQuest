...
type Reward = { id: string; name: string; description?: string; cost: number; is_active?: boolean };
...
function usePullToRefresh(onRefresh: () => Promise<void>) {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (event: TouchEvent) => {
      if (startY.current === null || window.scrollY > 0 || event.touches.length !== 1) return;
      const distance = event.touches[0].clientY - startY.current;
      if (distance <= 0) return;
      event.preventDefault();
      const nextDistance = Math.min(92, distance * 0.45);
...
function PullToRefreshIndicator({ pullDistance, isRefreshing }: { pullDistance: number; isRefreshing: boolean }) {
  ...
  const ready = pullDistance >= 56 || isRefreshing;
  return <div className={`pull-refresh-indicator ${ready ? 'ready' : ''}`} style={{ transform: `translate(-50%, ${isRefreshing ? 0 : Math.min(0, pullDistance - 58)}px)` }} role="status" aria-live="polite"><span className={isRefreshing ? 'spinning' : ''}>↻</span>{isRefreshing ? 'Načítám…' : ready ? 'Uvolněním obnovíte' : 'Táhněte pro obnovení'}</div>;
}
...
function NotificationCenter({ token }: { token: string }) {
  ...
  const load = async () => {
    const next = await request<Notification[]>('/notifications', {}, token);
    if (seen.current) {
      ...
      if (fresh.length && 'Notification' in window && Notification.permission === 'granted') {
        fresh.slice(0, 3).forEach((item) => new Notification(item.title, { body: item.message }));
      }
    }
...
function FamilyQuestMark() {
  return <svg className="familyquest-mark" viewBox="0 0 100 100" role="img" aria-label="FamilyQuest logo">
    <defs><linearGradient id="sidebar-mark-gradient" x1="15%" y1="0%" x2="85%" y2="100%"><stop stopColor="#d38cff" /><stop offset="1" stopColor="#5630ed" /></linearGradient></defs>
    <path fill="url(#sidebar-mark-gradient)" d="M50 3 12 38c-3 3-3 8 0 13l6 10V42l32-28 32 28v19l6-10c3-5 3-10 0-13L50 3Z" />
    <path fill="url(#sidebar-mark-gradient)" d="M22 45v22c0 5 3 9 7 12l21 14 21-14c4-3 7-7 7-12V45L50 22 22 45Z" />
    <path fill="#fff" d="M29 48v17c0 3 2 5 4 7l17 12 17-12c2-2 4-4 4-7V48L50 31 29 48Z" opacity=".14" />
    <g fill="#e0a5ff"><rect x="44" y="36" width="6" height="6" rx="1" /><rect x="52" y="36" width="6" height="6" rx="1" /><rect x="44" y="44" width="6" height="6" rx="1" /><rect x="52" y="44" width="6" height="6" rx="1" /></g>
    <circle cx="38" cy="57" r="7" fill="#f0d9ff" /><circle cx="62" cy="57" r="7" fill="#b96cff" /><circle cx="50" cy="67" r="5" fill="#d18cff" />
    <path fill="#170d48" d="m50 88-10-9c-8-7-7-14-2-18 5-3 10-1 12 4 2-5 7-7 12-4 5 4 6 11-2 18l-10 9Z" />
  </svg>;
}
...
function ParentDashboard({ user, token, onLogout }: { user: User; token: string; onLogout: () => void }) {
  ...
  const refresh = async () => {
    ...
    } catch (err) { setMessage(err instanceof Error ? err.message : 'Data se nepodařilo načíst.'); } finally { dataLoadingRef.current = false; setLoading(false); }
  };
  useEffect(() => { refresh(); }, []);
...
function AdventureMap({ points, totalXp, level, token, onClaim }: { points: AdventurePoint[]; totalXp: number; level: number; token?: string; onClaim?: () => Promise<void> }) {
...
function RewardManager({ rewards, token, onChange }: { rewards: Reward[]; token: string; onChange: () => void }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const data = new FormData(event.currentTarget);
    try {
      const payload = { name: String(data.get('name')), description: String(data.get('description') || '') || null, cost: Number(data.get('cost') || 1), is_active: data.get('is_active') === 'on' };
      await request(editing ? `/rewards/${editing.id}` : '/rewards', { method: editing ? 'PATCH' : 'POST', body: JSON.stringify(payload) }, token);
      setOpen(false); setEditing(null); setError(''); onChange();
    } catch (err) { setError(err instanceof Error ? err.message : 'Odměnu se nepodařilo uložit.'); }
  };
  return <section className="panel reward-manager" id="rewards"><div className="panel-heading"><div><p className="eyebrow">Nastavení gamifikace</p><h2>Odměny</h2></div><button className="primary compact" onClick={() => { setEditing(null); setOpen(true); setError(''); }}>+ Nová odměna</button></div>{open && <form className="achievement-form" onSubmit={save}><input name="name" required minLength={2} defaultValue={editing?.name ?? ''} placeholder="Název odměny" /><input name="description" defaultValue={editing?.description ?? ''} placeholder="Popis odměny" /><label>Cena v XP<input name="cost" type="number" min="1" defaultValue={editing?.cost ?? 100} /></label><label className="adventure-active"><input name="is_active" type="checkbox" defaultChecked={editing?.is_active ?? true} /> Aktivní</label>{error && <p className="error">{error}</p>}<div className="form-actions"><button type="button" className="secondary" onClick={() => { setOpen(false); setEditing(null); }}>Zrušit</button><button type="submit" className="primary">{editing ? 'Uložit změny' : 'Vytvořit odměnu'}</button></div></form>}{rewards.length === 0 ? <div className="empty">Zatím nemáte žádné odměny.</div> : <div className="achievement-admin-list">{rewards.map((reward) => <article className="achievement-admin-row" key={reward.id}><span className="achievement-admin-icon">🎁</span><div><strong>{reward.name}</strong><small>{reward.description || 'Rodinná odměna'} · {reward.cost} XP</small></div><div className="achievement-admin-actions"><button className="secondary compact" onClick={() => { setEditing(reward); setOpen(true); setError(''); }}>Upravit</button></div></article>)}</div>}</section>;
}
